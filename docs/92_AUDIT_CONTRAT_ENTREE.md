...
| `activitesCompatibles` (alias `activitesAutorisees`) | SalarieInputDTO | non | oui | oui | non | oui → `SalarieReel.activitesCompatibles` (Set.of() si null) | non | diagnostics : oui (pré-résolution SC-03 `aucuneRessourceDansDataset`) | tolérance | Aucune contrainte solveur active. **[S8.3]** Le contrat SC-03 l'annonçait SUPPORTÉ : corrigé en TOLÉRÉ. Le diagnostic pose d'ailleurs une question globale (« au moins une ressource du dataset le peut-elle ? ») et n'attrape donc pas une affectation individuelle incompatible. Arbitrage rendu : HARD, avec le lot des contraintes personnelles. En SC-06, `estEligible` l'applique déjà en filtre éliminatoire |
